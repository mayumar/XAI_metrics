...
class RelativeInputStability(BaseMetric):
    ...
    This metric evaluates the stability of explanations with respect to input
    perturbations. For each observation, Quantus generates perturbed inputs,
    recomputes their explanations with ``explain_func``, and computes the
    maximum ratio between the relative explanation change and the relative
    input change.

    The metric is based on the Relative Input Stability metric proposed by
    ...
    def run(self):
        ...
        to :class:`quantus.RelativeInputStability`. The model is set to evaluation
        ...
        -------
        List[float]
            Relative Input Stability score for each evaluated observation. Lower
            values indicate more stable explanations under relative input
            perturbations.
